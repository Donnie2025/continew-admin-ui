...
import { getEduCourseMaterialId, setEduLessonMaterial } from '@/apis/education/lesson'
...
const handleSave = async () => {
  saving.value = true
  try {
    await setEduLessonMaterial(lessonId.value, selectedId.value, selectedName.value)
...
const onOpen = async (id: string, name: string, cId: string, matId?: string | null, matName?: string | null) => {
  ...
  loading.value = true
  try {
    const pidRes = await getEduCourseMaterialId(cId)
