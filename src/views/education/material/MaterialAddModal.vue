...
import { createCloudFolder, renameCloudFile, renameCloudFolder } from '@/apis/education/classinCloud'
...
const save = async () => {
  try {
    ...
    if (isUpdate.value) {
      ...
      if (originalCloudId.value && form.name !== originalName.value) {
        try {
          if (originalType.value === 'LESSON') {
            const ext = originalCloudName.value?.match(/\.[^.]+$/)?.[0] ?? ''
            const baseName = ext ? form.name.replace(/\.[^.]+$/, '') : form.name
            await renameCloudFile(originalCloudId.value, baseName)
          } else {
            await renameCloudFolder(originalCloudId.value, form.name)
          }
        } catch {
          Message.warning('教材已更新，ClassIn重命名失败，请手动同步')
        }
      }
    } else {
      ...
      if (form.type !== 'LESSON' && parentCloudId.value && newId) {
        try {
          ...
          if (folderId) {
            await updateMaterial({ ...form, cloudId: folderId, cloudName: form.name }, String(newId))
          }
        } catch {
...
// 新增
const onAdd = (opts: Array<{ label: string; value: string }>, maxSort: number, pid: any, cloudId?: string | null) => {
  reset()
  dataId.value = ''
