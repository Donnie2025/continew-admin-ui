  ...
  <div class="gi_table_page">
    ...
    >
      ...
      <template #toolbar-right>
        <!-- 暂时屏蔽新增按钮 -->
        <!-- <a-button v-permission="['agent:course:create']" type="primary" @click="onAdd">
          <template #icon><icon-plus /></template>
          <template #default>新增</template>
        </a-button> 
        <a-button v-permission="['agent:course:export']" @click="onExport">
      ...
      <template #action="{ record }">
        <a-space>
          <a-link v-permission="['agent:course:get']" title="详情" @click="onDetail(record)">详情</a-link>
          <!-- <a-link v-permission="['agent:course:update']" title="修改" @click="onUpdate(record)">修改</a-link> -->
          <!-- <a-link title="关联老师" @click="onManageTeachers(record)">关联老师</a-link> -->
          <a-link title="关联学生" @click="onManageStudents(record)">学生</a-link>
          <a-link title="教材" @click="onSetMaterial(record)">教材</a-link>
          <a-link title="管理课节" @click="onManageLessons(record)">管理课节</a-link>
          <!-- <a-link
            v-permission="['agent:course:delete']"
            status="danger"
    ...
    <CourseMaterialModal ref="CourseMaterialModalRef" @save-success="search" />
...
import CourseMaterialModal from './CourseMaterialModal.vue'
import { type CourseResp, type CourseQuery, deleteCourse, exportCourse, listCourse, listCourseTeachers, listCourseStudents } from '@/apis/agent/course'
import { useDownload, useTable } from '@/hooks'
import { useDict } from '@/hooks/app'
import { isMobile } from '@/utils'
import has from '@/utils/has'

defineOptions({ name: 'AgentCourse' })
...
const columns: TableInstance['columns'] = [
  ...
  { title: '班主任', dataIndex: 'mainTeacherName', slotName: 'mainTeacherName', width: 100 },
  { title: '教师', dataIndex: 'teacherCount', slotName: 'teacherCount', width: 60 },
  { title: '班级学生', dataIndex: 'studentCount', slotName: 'studentCount', width: 220, align: 'center' },
  { title: '教材', dataIndex: 'materialName', slotName: 'materialName', width: 200, ellipsis: true, tooltip: true, align: 'left' },
  { title: '备注', dataIndex: 'remark', slotName: 'remark', width: 160, ellipsis: true, tooltip: true },
  {
    title: '操作',
    dataIndex: 'action',
    slotName: 'action',
    ...
    show: has.hasPermOr(['agent:course:get', 'agent:course:update', 'agent:course:delete'])
  }
]
...
const CourseMaterialModalRef = ref<InstanceType<typeof CourseMaterialModal>>()
// 设置教材
const onSetMaterial = (record: CourseResp) => {
  CourseMaterialModalRef.value?.onOpen(record.id, record.name, record.materialId ?? null, record.materialName ?? null, record.agentCode ?? null)
}
</script>
