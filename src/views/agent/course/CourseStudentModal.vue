...
const loadStudents = async () => {
  ...
  try {
    ...
    const agentCode = userStore.userInfo.agentCode

    if (keyword) {
      // 有搜索关键词时，使用搜索接口再按 agentCode 过滤
      const response = await searchStudent(keyword)
      const searchData = response?.data as any
      const all: StudentResp[] = Array.isArray(searchData?.data) ? searchData.data : []
      studentList = agentCode ? all.filter(s => s.agentCode === agentCode) : all
    } else {
      // 无搜索关键词时，直接经由 agentCode 参数过滤
